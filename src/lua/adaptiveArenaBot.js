    ...
    function decideNextAction()
        ...
        if player.health > 75 and player.energy > 50 then
            ...
            if distance <= 3 then
                ...
                ao.send({Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(math.min(player.energy, targetHealth))})
            else
        ...
        elseif player.health <= 75 or player.energy <= 20 then
            ...
            if distance <= 3 and player.energy > 30 then
                ...
                ao.send({Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = "30"})  -- Cautious attack
            else
    ...
    Handlers.add(
      ...
      function (msg)
        ...
        if playerHealth < 50 and playerEnergy > 30 then
          ...
          ao.send({Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = "30"})
        end
      end
    )
    `
