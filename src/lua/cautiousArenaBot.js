const cautiousArenaBot = {
    ...
    lua: `
    CautiousGameProcess = "0rVZYFxvfJpO__EfOz0_PUQ3GFE9kEaES0GkUDNXjvE"
...
function decideNextAction()
    ...
    if player.energy >= targetHealth and distance <= 3 then  -- Assuming attack range is 3
        ...
        ao.send({Target = CautiousGameProcess, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(player.energy)})
    else
        ...
        ao.send({Target = CautiousGameProcess, Action = "PlayerMove", Player = ao.id, Direction = moveDirection})
    end
    InAction = false
end
...
Handlers.add(
  ...
  function (msg)
    ...
    if playerEnergy >= 50 and playerHealth <= 50 then  -- Attack only if it has enough energy and low health
      ...
      ao.send({Target = CautiousGameProcess, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(playerEnergy)})
    end
  end
)
