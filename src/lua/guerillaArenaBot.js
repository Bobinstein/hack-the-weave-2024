...
function decideNextAction()
    ...
    if player.energy >= targetHealth and distance > 3 and player.energy > 50 then
        ...
        ao.send({Target = CautiousGameProcess, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(targetHealth)})  -- Use just enough energy to eliminate
    elseif distance <= 3 and player.energy > 20 then  -- If the enemy is close and has enough energy to attack
        ...
        ao.send({Target = CautiousGameProcess, Action = "PlayerAttack", Player = ao.id, AttackEnergy = "20"})  -- Attack with less power
        determineMoveDirection(player.x, player.y, targetX, targetY, true)  -- Move away after attacking
    else
        print(colors.blue .. "Moving to safe distance." .. colors.reset)
        determineMoveDirection(player.x, player.y, targetX, targetY, true)  -- Keep distance from closest player
    end
    InAction = false
end
...
function determineMoveDirection(x1, y1, x2, y2, avoid)
    ...
    ao.send({Target = CautiousGameProcess, Action = "PlayerMove", Player = ao.id, Direction = bestDirection})
end
...
Handlers.add(
  ...
  function (msg)
    ...
    if not InAction then
      InAction = true
      ao.send({Target = CautiousGameProcess, Action = "GetGameState"})
    end
  end
)
...
Handlers.add(
  ...
  function ()
    ...
    ao.send({ Target = CautiousGameProcess, Action = "Transfer", Recipient = CautiousGameProcess, Quantity = "1000"})
  end
)
...
Handlers.add(
  ...
  function (msg)
    ...
    if playerEnergy > 20 then
      ...
      ao.send({Target = CautiousGameProcess, Action = "PlayerAttack", Player = ao.id, AttackEnergy = "20"})
      determineMoveDirection(LatestGameState.Players[ao.id].x, LatestGameState.Players[ao.id].y, targetX, targetY, true)
    end
  end
)
