...
const BotSprite = ({ playerId, playerX, playerY, sectionSize, isOwned, player }) => {
    ...
    const positionStyle = {
        ...
        left: `${(playerX - 1) * 100 / sectionSize}%`,
        top: `${(playerY - 1) * 100 / sectionSize}%`,
        pointerEvents: 'auto',
    };
    ...
    return (
        ...
        >
            ...
            />
            <div style={tooltipStyle}>{`Player: ${playerId}\nHealth: ${player.health}\nEnergy: ${player.energy}`}</div>
        </div>
    );
};
